...
{
    // IM_ADJ_TRX_CELL
    public class ImAdjTrxCellConfiguration : IEntityTypeConfiguration<ImAdjTrxCell>
    {
        public void Configure(EntityTypeBuilder<ImAdjTrxCell> builder)
        {
            builder.ToTable("IM_ADJ_TRX_CELL", "dbo");
            builder.HasKey(x => new { x.BatId, x.ItemNo, x.LocId, x.TrxDat, x.SeqNo, x.Dim1Upr, x.Dim2Upr, x.Dim3Upr }).HasName("PK_IM_ADJ_TRX_CELL").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ImAdjTrx).WithMany(b => b.ImAdjTrxCells).HasForeignKey(c => new { c.BatId, c.ItemNo, c.LocId, c.TrxDat, c.SeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_ADJ_TRX_CELL_IM_ADJ_TRX");
        }
    }

...
{
    // IM_XFER_SER
    public class ImXferSerConfiguration : IEntityTypeConfiguration<ImXferSer>
    {
        public void Configure(EntityTypeBuilder<ImXferSer> builder)
        {
            builder.ToTable("IM_XFER_SER", "dbo");
            builder.HasKey(x => new { x.XferNo, x.XferLinSeqNo, x.SerNo }).HasName("PK_IM_XFER_SER").IsClustered();
            ...
            builder.Property(x => x.XferOutMiscAmt).HasColumnName(@"XFER_OUT_MISC_AMT").HasColumnType("decimal(15,2)").IsRequired(false);
            ...
            // Foreign keys
            builder.HasOne(a => a.ImXferLin).WithMany(b => b.ImXferSers).HasForeignKey(c => new { c.XferNo, c.XferLinSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_XFER_SER_IM_XFER_LIN");

            builder.HasIndex(x => new { x.XferNo, x.XferLinSeqNo, x.SerSeqNo }).HasDatabaseName("IM_XFER_SER_X1");
        }
    }

...
{
    // IM_QXFER_HIST_SER
    public class ImQxferHistSerConfiguration : IEntityTypeConfiguration<ImQxferHistSer>
    {
        public void Configure(EntityTypeBuilder<ImQxferHistSer> builder)
        {
            builder.ToTable("IM_QXFER_HIST_SER", "dbo");
            builder.HasKey(x => new { x.EventNo, x.BatId, x.ItemNo, x.FromLocId, x.ToLocId, x.TrxDat, x.SeqNo, x.SerNo }).HasName("PK_IM_QXFER_HIST_SER").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ImQxferHist).WithMany(b => b.ImQxferHistSers).HasForeignKey(c => new { c.EventNo, c.BatId, c.ItemNo, c.FromLocId, c.ToLocId, c.TrxDat, c.SeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_QXFER_HIST_SER_IM_QXFER_HIST");

            builder.HasIndex(x => new { x.EventNo, x.BatId, x.ItemNo, x.FromLocId, x.ToLocId, x.TrxDat, x.SeqNo, x.SerSeqNo }).HasDatabaseName("IM_QXFER_HIST_SER_X1");
        }
    }

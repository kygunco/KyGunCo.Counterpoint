...
{
    // PO_QRECV_HIST
    public class PoQrecvHistConfiguration : IEntityTypeConfiguration<PoQrecvHist>
    {
        public void Configure(EntityTypeBuilder<PoQrecvHist> builder)
        {
            builder.ToTable("PO_QRECV_HIST", "dbo");
            builder.HasKey(x => new { x.EventNo, x.BatId, x.ItemNo, x.LocId, x.TrxDat, x.SeqNo }).HasName("PK_PO_QRECV_HIST").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ImLocHist).WithMany(b => b.PoQrecvHists).HasForeignKey(c => c.LocId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_QRECV_HIST_IM_LOC_HIST");

            builder.HasIndex(x => new { x.LocId, x.EventNo, x.BatId, x.ItemNo, x.TrxDat, x.SeqNo }).HasDatabaseName("PO_QRECV_HIST_X1");
            builder.HasIndex(x => new { x.TrxDat, x.EventNo, x.BatId, x.ItemNo, x.LocId, x.SeqNo }).HasDatabaseName("PO_QRECV_HIST_X2");
        }
    }

...
{
    // AR_CATEG_NOTE
    public class ArCategNoteConfiguration : IEntityTypeConfiguration<ArCategNote>
    {
        public void Configure(EntityTypeBuilder<ArCategNote> builder)
        {
            builder.ToTable("AR_CATEG_NOTE", "dbo");
            builder.HasKey(x => new { x.CategCod, x.NoteId }).HasName("PK_AR_CATEG_NOTE").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ArCategCod).WithMany(b => b.ArCategNotes).HasForeignKey(c => c.CategCod).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_AR_CATEG_NOTE_AR_CATEG_COD");

            builder.HasIndex(x => new { x.CategCod, x.NoteDat, x.NoteId }).HasDatabaseName("AR_CATEG_NOTE_X1");
        }
    }

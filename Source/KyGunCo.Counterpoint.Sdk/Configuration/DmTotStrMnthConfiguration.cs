...
{
    // DM_TOT_STR_MNTH
    public class DmTotStrMnthConfiguration : IEntityTypeConfiguration<DmTotStrMnth>
    {
        public void Configure(EntityTypeBuilder<DmTotStrMnth> builder)
        {
            builder.ToTable("DM_TOT_STR_MNTH", "dbo");
            builder.HasKey(x => new { x.MnthId, x.StrId }).HasName("PK_DM_TOT_STR_MNTH").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.DmStrHist).WithMany(b => b.DmTotStrMnths).HasForeignKey(c => c.StrId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_DM_TOT_STR_MNTH_DM_STR_HIST");

            builder.HasIndex(x => new { x.StrId, x.MnthId }).HasDatabaseName("DM_TOT_STR_MNTH_X1");
        }
    }

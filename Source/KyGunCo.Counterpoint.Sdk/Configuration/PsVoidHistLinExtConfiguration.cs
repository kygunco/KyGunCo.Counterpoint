...
{
    // PS_VOID_HIST_LIN_EXT
    public class PsVoidHistLinExtConfiguration : IEntityTypeConfiguration<PsVoidHistLinExt>
    {
        public void Configure(EntityTypeBuilder<PsVoidHistLinExt> builder)
        {
            builder.ToTable("PS_VOID_HIST_LIN_EXT", "dbo");
            builder.HasKey(x => new { x.BusDatExt, x.DocIdExt, x.LinSeqNoExt }).HasName("PK_PS_VOID_HIST_LIN_EXT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsVoidHistLin).WithOne(b => b.PsVoidHistLinExt).HasForeignKey<PsVoidHistLinExt>(c => new { c.BusDatExt, c.DocIdExt, c.LinSeqNoExt }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_VOID_HIST_LIN_EXT_PS_VOID_HIST_LIN");
        }
    }

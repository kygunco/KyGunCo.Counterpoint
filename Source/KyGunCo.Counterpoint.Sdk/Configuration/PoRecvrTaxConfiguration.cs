...
{
    // PO_RECVR_TAX
    public class PoRecvrTaxConfiguration : IEntityTypeConfiguration<PoRecvrTax>
    {
        public void Configure(EntityTypeBuilder<PoRecvrTax> builder)
        {
            builder.ToTable("PO_RECVR_TAX", "dbo");
            builder.HasKey(x => new { x.RecvrNo, x.AuthCod, x.RulCod }).HasName("PK_PO_RECVR_TAX").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PoRecvrHdr).WithMany(b => b.PoRecvrTaxes).HasForeignKey(c => c.RecvrNo).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_RECVR_TAX_PO_RECVR_HDR");
        }
    }

...
{
    // PO_ADJ_TAX
    public class PoAdjTaxConfiguration : IEntityTypeConfiguration<PoAdjTax>
    {
        public void Configure(EntityTypeBuilder<PoAdjTax> builder)
        {
            builder.ToTable("PO_ADJ_TAX", "dbo");
            builder.HasKey(x => new { x.RecvrNo, x.AuthCod, x.RulCod, x.TaxDetlTyp }).HasName("PK_PO_ADJ_TAX").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PoAdjHdr).WithMany(b => b.PoAdjTaxes).HasForeignKey(c => c.RecvrNo).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_ADJ_TAX_PO_ADJ_HDR");
        }
    }

...
{
    // IM_QXFER_TRX_SER
    public class ImQxferTrxSerConfiguration : IEntityTypeConfiguration<ImQxferTrxSer>
    {
        public void Configure(EntityTypeBuilder<ImQxferTrxSer> builder)
        {
            builder.ToTable("IM_QXFER_TRX_SER", "dbo");
            builder.HasKey(x => new { x.BatId, x.ItemNo, x.FromLocId, x.ToLocId, x.TrxDat, x.SeqNo, x.SerNo }).HasName("PK_IM_QXFER_TRX_SER").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ImQxferTrx).WithMany(b => b.ImQxferTrxSers).HasForeignKey(c => new { c.BatId, c.ItemNo, c.FromLocId, c.ToLocId, c.TrxDat, c.SeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_QXFER_TRX_SER_IM_QXFER_TRX");

            builder.HasIndex(x => new { x.BatId, x.ItemNo, x.FromLocId, x.ToLocId, x.TrxDat, x.SeqNo, x.SerSeqNo }).HasDatabaseName("IM_QXFER_TRX_SER_X1");
        }
    }

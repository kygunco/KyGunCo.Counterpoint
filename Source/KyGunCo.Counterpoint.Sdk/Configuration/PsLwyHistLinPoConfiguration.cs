...
{
    // PS_LWY_HIST_LIN_PO
    public class PsLwyHistLinPoConfiguration : IEntityTypeConfiguration<PsLwyHistLinPo>
    {
        public void Configure(EntityTypeBuilder<PsLwyHistLinPo> builder)
        {
            builder.ToTable("PS_LWY_HIST_LIN_PO", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LinSeqNo }).HasName("PK_PS_LWY_HIST_LIN_PO").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsLwyHistLin).WithOne(b => b.PsLwyHistLinPo).HasForeignKey<PsLwyHistLinPo>(c => new { c.BusDat, c.DocId, c.LinSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_LWY_HIST_LIN_PO_PS_LWY_HIST_LIN");
        }
    }

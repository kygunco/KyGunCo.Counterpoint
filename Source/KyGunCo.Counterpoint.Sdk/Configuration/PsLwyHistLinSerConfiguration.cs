...
{
    // PS_LWY_HIST_LIN_SER
    public class PsLwyHistLinSerConfiguration : IEntityTypeConfiguration<PsLwyHistLinSer>
    {
        public void Configure(EntityTypeBuilder<PsLwyHistLinSer> builder)
        {
            builder.ToTable("PS_LWY_HIST_LIN_SER", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LinSeqNo, x.SerNo }).HasName("PK_PS_LWY_HIST_LIN_SER").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsLwyHistLin).WithMany(b => b.PsLwyHistLinSers).HasForeignKey(c => new { c.BusDat, c.DocId, c.LinSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_LWY_HIST_LIN_SER_PS_LWY_HIST_LIN");
        }
    }

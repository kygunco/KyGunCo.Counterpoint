...
{
    // PS_ORD_HIST_PMT
    public class PsOrdHistPmtConfiguration : IEntityTypeConfiguration<PsOrdHistPmt>
    {
        public void Configure(EntityTypeBuilder<PsOrdHistPmt> builder)
        {
            builder.ToTable("PS_ORD_HIST_PMT", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.PmtSeqNo }).HasName("PK_PS_ORD_HIST_PMT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsOrdHist).WithMany(b => b.PsOrdHistPmts).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_ORD_HIST_PMT_PS_ORD_HIST");

            builder.HasIndex(x => new { x.StrId, x.StaId, x.TktNo, x.PmtSeqNo }).HasDatabaseName("PS_ORD_HIST_PMT_X1");
        }
    }

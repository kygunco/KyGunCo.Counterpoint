...
{
    // PS_LWY_HIST_CONTACT
    public class PsLwyHistContactConfiguration : IEntityTypeConfiguration<PsLwyHistContact>
    {
        public void Configure(EntityTypeBuilder<PsLwyHistContact> builder)
        {
            builder.ToTable("PS_LWY_HIST_CONTACT", "dbo");
            builder.HasKey(x => new { x.DocId, x.BusDat, x.ContactId }).HasName("PK_PS_LWY_HIST_CONTACT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsLwyHist).WithMany(b => b.PsLwyHistContacts).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_LWY_HIST_CONTACT_PS_LWY_HIST");

            builder.HasIndex(x => new { x.LstNam, x.FstNam }).HasDatabaseName("PS_LWY_HIST_CONTACT_X1");
            builder.HasIndex(x => x.Nam).HasDatabaseName("PS_LWY_HIST_CONTACT_X2");
        }
    }

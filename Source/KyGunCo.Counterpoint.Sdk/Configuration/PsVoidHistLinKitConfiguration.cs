...
{
    // PS_VOID_HIST_LIN_KIT
    public class PsVoidHistLinKitConfiguration : IEntityTypeConfiguration<PsVoidHistLinKit>
    {
        public void Configure(EntityTypeBuilder<PsVoidHistLinKit> builder)
        {
            builder.ToTable("PS_VOID_HIST_LIN_KIT", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LinSeqNo }).HasName("PK_PS_VOID_HIST_LIN_KIT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsVoidHistLin).WithOne(b => b.PsVoidHistLinKit).HasForeignKey<PsVoidHistLinKit>(c => new { c.BusDat, c.DocId, c.LinSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_VOID_HIST_LIN_KIT_PS_VOID_HIST_LIN");
        }
    }

...
{
    // PO_RECVR_HIST_NOTE
    public class PoRecvrHistNoteConfiguration : IEntityTypeConfiguration<PoRecvrHistNote>
    {
        public void Configure(EntityTypeBuilder<PoRecvrHistNote> builder)
        {
            builder.ToTable("PO_RECVR_HIST_NOTE", "dbo");
            builder.HasKey(x => new { x.RecvrNo, x.NoteId }).HasName("PK_PO_RECVR_HIST_NOTE").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PoRecvrHist).WithMany(b => b.PoRecvrHistNotes).HasForeignKey(c => c.RecvrNo).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_RECVR_HIST_NOTE_PO_RECVR_HIST");

            builder.HasIndex(x => new { x.RecvrNo, x.NoteDat, x.NoteId }).HasDatabaseName("PO_RECVR_HIST_NOTE_X1");
        }
    }

...
{
    // IM_XFER_NOTE
    public class ImXferNoteConfiguration : IEntityTypeConfiguration<ImXferNote>
    {
        public void Configure(EntityTypeBuilder<ImXferNote> builder)
        {
            builder.ToTable("IM_XFER_NOTE", "dbo");
            builder.HasKey(x => new { x.XferNo, x.NoteId }).HasName("PK_IM_XFER_NOTE").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ImXferHdr).WithMany(b => b.ImXferNotes).HasForeignKey(c => c.XferNo).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_XFER_NOTE_IM_XFER_HDR");

            builder.HasIndex(x => new { x.XferNo, x.NoteDat, x.NoteId }).HasDatabaseName("IM_XFER_NOTE_X1");
        }
    }

...
{
    // PS_TKT_HIST_GFC
    public class PsTktHistGfcConfiguration : IEntityTypeConfiguration<PsTktHistGfc>
    {
        public void Configure(EntityTypeBuilder<PsTktHistGfc> builder)
        {
            builder.ToTable("PS_TKT_HIST_GFC", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.GfcSeqNo }).HasName("PK_PS_TKT_HIST_GFC").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsTktHist).WithMany(b => b.PsTktHistGfcs).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_TKT_HIST_GFC_PS_TKT_HIST");
        }
    }

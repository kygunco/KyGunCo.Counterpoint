...
{
    // PS_VOID_HIST_AUDIT_LOG_TOT
    public class PsVoidHistAuditLogTotConfiguration : IEntityTypeConfiguration<PsVoidHistAuditLogTot>
    {
        public void Configure(EntityTypeBuilder<PsVoidHistAuditLogTot> builder)
        {
            builder.ToTable("PS_VOID_HIST_AUDIT_LOG_TOT", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LogSeqNo, x.TotTyp }).HasName("PK_PS_VOID_HIST_AUDIT_LOG_TOT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsVoidHistAuditLog).WithMany(b => b.PsVoidHistAuditLogTots).HasForeignKey(c => new { c.BusDat, c.DocId, c.LogSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_VOID_HIST_AUDIT_LOG_TOT_PS_VOID_HIST_AUDIT_LOG");
        }
    }

...
{
    // DM_TOT_STR_HR
    public class DmTotStrHrConfiguration : IEntityTypeConfiguration<DmTotStrHr>
    {
        public void Configure(EntityTypeBuilder<DmTotStrHr> builder)
        {
            builder.ToTable("DM_TOT_STR_HR", "dbo");
            builder.HasKey(x => new { x.PostDat, x.StrId, x.HrId }).HasName("PK_DM_TOT_STR_HR").IsClustered();
            ...
            builder.Property(x => x.HrId).HasColumnName(@"HR_ID").HasColumnType("tinyint").IsRequired().ValueGeneratedNever();
            ...
            // Foreign keys
            builder.HasOne(a => a.DmStrHist).WithMany(b => b.DmTotStrHrs).HasForeignKey(c => c.StrId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_DM_TOT_STR_HR_DM_STR_HIST");

            builder.HasIndex(x => new { x.StrId, x.PostDat, x.HrId }).HasDatabaseName("DM_TOT_STR_HR_X1");
        }
    }

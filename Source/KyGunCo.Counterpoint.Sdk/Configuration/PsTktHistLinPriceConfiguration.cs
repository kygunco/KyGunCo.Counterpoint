...
{
    // PS_TKT_HIST_LIN_PRICE
    public class PsTktHistLinPriceConfiguration : IEntityTypeConfiguration<PsTktHistLinPrice>
    {
        public void Configure(EntityTypeBuilder<PsTktHistLinPrice> builder)
        {
            builder.ToTable("PS_TKT_HIST_LIN_PRICE", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LinSeqNo, x.PrcSeqNo }).HasName("PK_PS_TKT_HIST_LIN_PRICE").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsTktHistLin).WithMany(b => b.PsTktHistLinPrices).HasForeignKey(c => new { c.BusDat, c.DocId, c.LinSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_TKT_HIST_LIN_PRICE_PS_TKT_HIST_LIN");
        }
    }

...
{
    // AR_OPN_ITEM
    public class ArOpnItemConfiguration : IEntityTypeConfiguration<ArOpnItem>
    {
        public void Configure(EntityTypeBuilder<ArOpnItem> builder)
        {
            builder.ToTable("AR_OPN_ITEM", "dbo");
            builder.HasKey(x => new { x.CustNo, x.DocDat, x.DocNo, x.DocTyp }).HasName("PK_AR_OPN_ITEM").IsClustered();
            ...
            builder.Property(x => x.LstReapplDt).HasColumnName(@"LST_REAPPL_DT").HasColumnType("datetime").IsRequired(false);
            builder.Property(x => x.LstReapplUsrId).HasColumnName(@"LST_REAPPL_USR_ID").HasColumnType("varchar(10)").IsRequired(false).IsUnicode(false).HasMaxLength(10);
            ...
            // Foreign keys
            builder.HasOne(a => a.ArCust).WithMany(b => b.ArOpnItems).HasForeignKey(c => c.CustNo).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_AR_OPN_ITEM_AR_CUST");

            builder.HasIndex(x => new { x.CustNo, x.DueDat, x.DocDat, x.DocNo, x.DocTyp }).HasDatabaseName("AR_OPN_ITEM_X1");
            builder.HasIndex(x => new { x.CustNo, x.DocNo, x.DocDat, x.DocTyp }).HasDatabaseName("AR_OPN_ITEM_X3");
            builder.HasIndex(x => new { x.CustNo, x.DocTyp, x.DocDat, x.DocNo }).HasDatabaseName("AR_OPN_ITEM_X4");
            builder.HasIndex(x => new { x.DocNo, x.CustNo, x.DocDat, x.DocTyp }).HasDatabaseName("AR_OPN_ITEM_X5");
            builder.HasIndex(x => new { x.SelfApplied, x.CustNo, x.DocDat, x.DocNo, x.DocTyp }).HasDatabaseName("AR_OPN_ITEM_X6");
        }
    }

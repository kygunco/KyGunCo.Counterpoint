...
{
    // PS_ORD_HIST_NOTE
    public class PsOrdHistNoteConfiguration : IEntityTypeConfiguration<PsOrdHistNote>
    {
        public void Configure(EntityTypeBuilder<PsOrdHistNote> builder)
        {
            builder.ToTable("PS_ORD_HIST_NOTE", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.NoteSeqNo }).HasName("PK_PS_ORD_HIST_NOTE").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsOrdHist).WithMany(b => b.PsOrdHistNotes).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_ORD_HIST_NOTE_PS_ORD_HIST");
        }
    }

...
{
    // IM_SUBCAT_COD
    public class ImSubcatCodConfiguration : IEntityTypeConfiguration<ImSubcatCod>
    {
        public void Configure(EntityTypeBuilder<ImSubcatCod> builder)
        {
            builder.ToTable("IM_SUBCAT_COD", "dbo");
            builder.HasKey(x => new { x.SubcatCod, x.CategCod }).HasName("PK_IM_SUBCAT_COD").IsClustered();
            ...
            builder.HasIndex(x => x.RsStat).HasDatabaseName("IM_SUBCAT_COD_X_RS_STAT");
            builder.HasIndex(x => x.RsUtcDt).HasDatabaseName("IM_SUBCAT_COD_X_RS_UTC_DT");
            builder.HasIndex(x => new { x.DescrUpr, x.SubcatCod, x.CategCod }).HasDatabaseName("IM_SUBCAT_COD_X1");
            builder.HasIndex(x => new { x.CategCod, x.SubcatCod }).HasDatabaseName("IM_SUBCAT_COD_X2");
        }
    }

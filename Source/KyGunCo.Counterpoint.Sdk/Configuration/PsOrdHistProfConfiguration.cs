...
{
    // PS_ORD_HIST_PROF
    public class PsOrdHistProfConfiguration : IEntityTypeConfiguration<PsOrdHistProf>
    {
        public void Configure(EntityTypeBuilder<PsOrdHistProf> builder)
        {
            builder.ToTable("PS_ORD_HIST_PROF", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId }).HasName("PK_PS_ORD_HIST_PROF").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsOrdHist).WithOne(b => b.PsOrdHistProf).HasForeignKey<PsOrdHistProf>(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_ORD_HIST_PROF_PS_ORD_HIST");
        }
    }

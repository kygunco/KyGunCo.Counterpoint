...
{
    // IM_XFER_RECON_HIST_NOTE
    public class ImXferReconHistNoteConfiguration : IEntityTypeConfiguration<ImXferReconHistNote>
    {
        public void Configure(EntityTypeBuilder<ImXferReconHistNote> builder)
        {
            builder.ToTable("IM_XFER_RECON_HIST_NOTE", "dbo");
            builder.HasKey(x => new { x.XferNo, x.NoteId }).HasName("PK_IM_XFER_RECON_HIST_NOTE").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ImXferReconHist).WithMany(b => b.ImXferReconHistNotes).HasForeignKey(c => c.XferNo).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_XFER_RECON_HIST_NOTE_IM_XFER_RECON_HIST");

            builder.HasIndex(x => new { x.XferNo, x.NoteDat, x.NoteId }).HasDatabaseName("IM_XFER_RECON_HIST_NOTE_X1");
        }
    }

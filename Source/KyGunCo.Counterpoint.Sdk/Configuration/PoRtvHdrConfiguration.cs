...
{
    // PO_RTV_HDR
    public class PoRtvHdrConfiguration : IEntityTypeConfiguration<PoRtvHdr>
    {
        public void Configure(EntityTypeBuilder<PoRtvHdr> builder)
        {
            builder.ToTable("PO_RTV_HDR", "dbo");
            builder.HasKey(x => x.RtvNo).HasName("PK_PO_RTV_HDR").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ArTaxCod).WithMany(b => b.PoRtvHdrs).HasForeignKey(c => c.TaxCod).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_RTV_HDR_AR_TAX_COD");
            builder.HasOne(a => a.ImLoc).WithMany(b => b.PoRtvHdrs).HasForeignKey(c => c.RtvLocId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_RTV_HDR_IM_LOC");

            builder.HasIndex(x => new { x.VendNo, x.RtvNo }).HasDatabaseName("PO_RTV_HDR_X1");
            builder.HasIndex(x => new { x.UsrId, x.RtvNo }).HasDatabaseName("PO_RTV_HDR_X2");
            builder.HasIndex(x => new { x.RtvLocId, x.RtvNo }).HasDatabaseName("PO_RTV_HDR_X3");
            builder.HasIndex(x => new { x.VendNamUpr, x.RtvNo }).HasDatabaseName("PO_RTV_HDR_X4");
            builder.HasIndex(x => new { x.VendFstNamUpr, x.RtvNo }).HasDatabaseName("PO_RTV_HDR_X5");
            builder.HasIndex(x => new { x.VendLstNamUpr, x.RtvNo }).HasDatabaseName("PO_RTV_HDR_X6");
        }
    }

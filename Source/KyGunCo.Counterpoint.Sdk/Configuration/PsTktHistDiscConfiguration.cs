...
{
    // PS_TKT_HIST_DISC
    public class PsTktHistDiscConfiguration : IEntityTypeConfiguration<PsTktHistDisc>
    {
        public void Configure(EntityTypeBuilder<PsTktHistDisc> builder)
        {
            builder.ToTable("PS_TKT_HIST_DISC", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.DiscSeqNo }).HasName("PK_PS_TKT_HIST_DISC").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsTktHist).WithMany(b => b.PsTktHistDiscs).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_TKT_HIST_DISC_PS_TKT_HIST");
            builder.HasOne(a => a.PsTktHistLin).WithMany(b => b.PsTktHistDiscs).HasForeignKey(c => new { c.BusDat, c.DocId, c.LinSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_TKT_HIST_DISC_PS_TKT_HIST_LIN");
        }
    }

...
{
    // PS_DOC_PMT_CHK
    public class PsDocPmtChkConfiguration : IEntityTypeConfiguration<PsDocPmtChk>
    {
        public void Configure(EntityTypeBuilder<PsDocPmtChk> builder)
        {
            builder.ToTable("PS_DOC_PMT_CHK", "dbo");
            builder.HasKey(x => new { x.DocId, x.PmtSeqNo }).HasName("PK_PS_DOC_PMT_CHK").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsDocPmt).WithOne(b => b.PsDocPmtChk).HasForeignKey<PsDocPmtChk>(c => new { c.DocId, c.PmtSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_DOC_PMT_CHK_PS_DOC_PMT");
        }
    }

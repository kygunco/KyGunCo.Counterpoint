...
{
    // IM_XFER_OUT_LIN
    public class ImXferOutLinConfiguration : IEntityTypeConfiguration<ImXferOutLin>
    {
        public void Configure(EntityTypeBuilder<ImXferOutLin> builder)
        {
            builder.ToTable("IM_XFER_OUT_LIN", "dbo");
            builder.HasKey(x => new { x.XferNo, x.XferLinSeqNo }).HasName("PK_IM_XFER_OUT_LIN").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ImXferOutHdr).WithMany(b => b.ImXferOutLins).HasForeignKey(c => c.XferNo).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_XFER_OUT_LIN_IM_XFER_OUT_HDR");

            builder.HasIndex(x => new { x.ItemNo, x.XferNo, x.XferLinSeqNo }).HasDatabaseName("IM_XFER_OUT_LIN_X1");
        }
    }

...
{
    // PS_ORD_HIST_LIN_SER_PROMPT
    public class PsOrdHistLinSerPromptConfiguration : IEntityTypeConfiguration<PsOrdHistLinSerPrompt>
    {
        public void Configure(EntityTypeBuilder<PsOrdHistLinSerPrompt> builder)
        {
            builder.ToTable("PS_ORD_HIST_LIN_SER_PROMPT", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LinSeqNo, x.SerNo, x.PromptSeqNo }).HasName("PK_PS_ORD_HIST_LIN_SER_PROMPT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsOrdHistLinSer).WithMany(b => b.PsOrdHistLinSerPrompts).HasForeignKey(c => new { c.BusDat, c.DocId, c.LinSeqNo, c.SerNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_ORD_HIST_LIN_SER_PROMPT_PS_ORD_HIST_LIN_SER");
        }
    }

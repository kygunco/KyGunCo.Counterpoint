...
{
    // PS_ORD_HIST_LIN_KIT
    public class PsOrdHistLinKitConfiguration : IEntityTypeConfiguration<PsOrdHistLinKit>
    {
        public void Configure(EntityTypeBuilder<PsOrdHistLinKit> builder)
        {
            builder.ToTable("PS_ORD_HIST_LIN_KIT", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LinSeqNo }).HasName("PK_PS_ORD_HIST_LIN_KIT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsOrdHistLin).WithOne(b => b.PsOrdHistLinKit).HasForeignKey<PsOrdHistLinKit>(c => new { c.BusDat, c.DocId, c.LinSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_ORD_HIST_LIN_KIT_PS_ORD_HIST_LIN");
        }
    }

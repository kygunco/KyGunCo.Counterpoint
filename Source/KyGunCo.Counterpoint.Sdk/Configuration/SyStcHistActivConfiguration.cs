...
{
    // SY_STC_HIST_ACTIV
    public class SyStcHistActivConfiguration : IEntityTypeConfiguration<SyStcHistActiv>
    {
        public void Configure(EntityTypeBuilder<SyStcHistActiv> builder)
        {
            builder.ToTable("SY_STC_HIST_ACTIV", "dbo");
            builder.HasKey(x => new { x.EventNo, x.StcNo, x.SeqNo }).HasName("PK_SY_STC_HIST_ACTIV").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.SyStcHist).WithMany(b => b.SyStcHistActivs).HasForeignKey(c => new { c.EventNo, c.StcNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_SY_STC_HIST_ACTIV_SY_STC_HIST");
        }
    }

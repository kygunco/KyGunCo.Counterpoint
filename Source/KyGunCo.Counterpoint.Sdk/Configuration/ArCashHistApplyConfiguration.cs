...
{
    // AR_CASH_HIST_APPLY
    public class ArCashHistApplyConfiguration : IEntityTypeConfiguration<ArCashHistApply>
    {
        public void Configure(EntityTypeBuilder<ArCashHistApply> builder)
        {
            builder.ToTable("AR_CASH_HIST_APPLY", "dbo");
            builder.HasKey(x => new { x.EventNo, x.CustNo, x.DocNo, x.ApplyToDocDat, x.ApplyToDocNo, x.ApplyToDocTyp }).HasName("PK_AR_CASH_HIST_APPLY").IsClustered();
            ...
            builder.Property(x => x.DiscAcctNoUsed).HasColumnName(@"DISC_ACCT_NO_USED").HasColumnType("varchar(20)").IsRequired(false).IsUnicode(false).HasMaxLength(20);
            ...
            builder.Property(x => x.WrtoffAcctNoUsed).HasColumnName(@"WRTOFF_ACCT_NO_USED").HasColumnType("varchar(20)").IsRequired(false).IsUnicode(false).HasMaxLength(20);
            ...
            // Foreign keys
            builder.HasOne(a => a.ArCashHist).WithMany(b => b.ArCashHistApplies).HasForeignKey(c => new { c.EventNo, c.CustNo, c.DocNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_AR_CASH_HIST_APPLY_AR_CASH_HIST");

            builder.HasIndex(x => new { x.EventNo, x.CustNo, x.DocNo, x.SeqNo }).HasDatabaseName("AR_CASH_HIST_APPLY_X1");
        }
    }

...
{
    // IM_ADJ_HIST_SER
    public class ImAdjHistSerConfiguration : IEntityTypeConfiguration<ImAdjHistSer>
    {
        public void Configure(EntityTypeBuilder<ImAdjHistSer> builder)
        {
            builder.ToTable("IM_ADJ_HIST_SER", "dbo");
            builder.HasKey(x => new { x.EventNo, x.BatId, x.ItemNo, x.LocId, x.TrxDat, x.SeqNo, x.SerNo }).HasName("PK_IM_ADJ_HIST_SER").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ImAdjHist).WithMany(b => b.ImAdjHistSers).HasForeignKey(c => new { c.EventNo, c.BatId, c.ItemNo, c.LocId, c.TrxDat, c.SeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_ADJ_HIST_SER_IM_ADJ_HIST");

            builder.HasIndex(x => new { x.EventNo, x.BatId, x.ItemNo, x.LocId, x.TrxDat, x.SeqNo, x.SerSeqNo }).HasDatabaseName("IM_ADJ_HIST_SER_X1");
            builder.HasIndex(x => new { x.BatId, x.ItemNo, x.LocId, x.TrxDat, x.SeqNo }).HasDatabaseName("USER_IX_IM_ADJ_HIST_SER_BAT_ID_ITEM_NO_LOC_ID_TRX_DAT_SEQ_NO");
            builder.HasIndex(x => new { x.ItemNo, x.LocId }).HasDatabaseName("USER_IX_IM_ADJ_HIST_SER_ITEM_NO_LOC_ID");
        }
    }

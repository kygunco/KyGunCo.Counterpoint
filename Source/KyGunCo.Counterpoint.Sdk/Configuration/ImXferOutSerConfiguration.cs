...
{
    // IM_XFER_OUT_SER
    public class ImXferOutSerConfiguration : IEntityTypeConfiguration<ImXferOutSer>
    {
        public void Configure(EntityTypeBuilder<ImXferOutSer> builder)
        {
            builder.ToTable("IM_XFER_OUT_SER", "dbo");
            builder.HasKey(x => new { x.XferNo, x.XferLinSeqNo, x.SerNo }).HasName("PK_IM_XFER_OUT_SER").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ImXferOutLin).WithMany(b => b.ImXferOutSers).HasForeignKey(c => new { c.XferNo, c.XferLinSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_XFER_OUT_SER_IM_XFER_OUT_LIN");

            builder.HasIndex(x => new { x.XferNo, x.XferLinSeqNo, x.SerSeqNo }).HasDatabaseName("IM_XFER_OUT_SER_X1");
        }
    }

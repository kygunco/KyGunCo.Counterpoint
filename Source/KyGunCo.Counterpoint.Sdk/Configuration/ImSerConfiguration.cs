...
{
    // IM_SER
    public class ImSerConfiguration : IEntityTypeConfiguration<ImSer>
    {
        public void Configure(EntityTypeBuilder<ImSer> builder)
        {
            builder.ToTable("IM_SER", "dbo");
            builder.HasKey(x => new { x.ItemNo, x.SerNo }).HasName("PK_IM_SER").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ImItem).WithMany(b => b.ImSers).HasForeignKey(c => c.ItemNo).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_SER_IM_ITEM");

            builder.HasIndex(x => new { x.ItemNo, x.LocId, x.Stat, x.SerNo }).HasDatabaseName("IM_SER_X1");
            builder.HasIndex(x => new { x.SerNo, x.ItemNo }).HasDatabaseName("IM_SER_X2");
            builder.HasIndex(x => new { x.ItemNo, x.Stat, x.SerNo }).HasDatabaseName("IM_SER_X3");
        }
    }

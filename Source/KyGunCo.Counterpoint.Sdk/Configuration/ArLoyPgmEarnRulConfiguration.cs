...
{
    // AR_LOY_PGM_EARN_RUL
    public class ArLoyPgmEarnRulConfiguration : IEntityTypeConfiguration<ArLoyPgmEarnRul>
    {
        public void Configure(EntityTypeBuilder<ArLoyPgmEarnRul> builder)
        {
            builder.ToTable("AR_LOY_PGM_EARN_RUL", "dbo");
            builder.HasKey(x => new { x.LoyPgmCod, x.RulSeqNo }).HasName("PK_AR_LOY_PGM_EARN_RUL").IsClustered();
            ...
            builder.Property(x => x.LoyPtsEarnPerAmt).HasColumnName(@"LOY_PTS_EARN_PER_AMT").HasColumnType("decimal(15,4)").IsRequired();
            builder.Property(x => x.LoyPtsEarnPerUnit).HasColumnName(@"LOY_PTS_EARN_PER_UNIT").HasColumnType("decimal(15,4)").IsRequired();
            ...
            // Foreign keys
            builder.HasOne(a => a.ArLoyPgm).WithMany(b => b.ArLoyPgmEarnRuls).HasForeignKey(c => c.LoyPgmCod).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_AR_LOY_PGM_EARN_RUL_AR_LOY_PGM");

            builder.HasIndex(x => new { x.DescrUpr, x.LoyPgmCod, x.RulSeqNo }).HasDatabaseName("AR_LOY_PGM_EARN_RUL_X1");
        }
    }

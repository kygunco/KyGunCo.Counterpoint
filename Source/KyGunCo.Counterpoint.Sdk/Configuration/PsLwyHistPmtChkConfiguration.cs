...
{
    // PS_LWY_HIST_PMT_CHK
    public class PsLwyHistPmtChkConfiguration : IEntityTypeConfiguration<PsLwyHistPmtChk>
    {
        public void Configure(EntityTypeBuilder<PsLwyHistPmtChk> builder)
        {
            builder.ToTable("PS_LWY_HIST_PMT_CHK", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.PmtSeqNo }).HasName("PK_PS_LWY_HIST_PMT_CHK").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsLwyHistPmt).WithOne(b => b.PsLwyHistPmtChk).HasForeignKey<PsLwyHistPmtChk>(c => new { c.BusDat, c.DocId, c.PmtSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_LWY_HIST_PMT_CHK_PS_LWY_HIST_PMT");
        }
    }

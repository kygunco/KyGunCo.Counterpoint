...
{
    // IM_XFER_RECON_CELL
    public class ImXferReconCellConfiguration : IEntityTypeConfiguration<ImXferReconCell>
    {
        public void Configure(EntityTypeBuilder<ImXferReconCell> builder)
        {
            builder.ToTable("IM_XFER_RECON_CELL", "dbo");
            builder.HasKey(x => new { x.XferNo, x.XferLinSeqNo, x.Dim1Upr, x.Dim2Upr, x.Dim3Upr }).HasName("PK_IM_XFER_RECON_CELL").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ImXferReconLin).WithMany(b => b.ImXferReconCells).HasForeignKey(c => new { c.XferNo, c.XferLinSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_XFER_RECON_CELL_IM_XFER_RECON_LIN");
        }
    }

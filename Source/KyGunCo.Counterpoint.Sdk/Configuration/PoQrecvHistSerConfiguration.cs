...
{
    // PO_QRECV_HIST_SER
    public class PoQrecvHistSerConfiguration : IEntityTypeConfiguration<PoQrecvHistSer>
    {
        public void Configure(EntityTypeBuilder<PoQrecvHistSer> builder)
        {
            builder.ToTable("PO_QRECV_HIST_SER", "dbo");
            builder.HasKey(x => new { x.EventNo, x.BatId, x.ItemNo, x.LocId, x.TrxDat, x.SeqNo, x.SerNo }).HasName("PK_PO_QRECV_HIST_SER").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PoQrecvHist).WithMany(b => b.PoQrecvHistSers).HasForeignKey(c => new { c.EventNo, c.BatId, c.ItemNo, c.LocId, c.TrxDat, c.SeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_QRECV_HIST_SER_PO_QRECV_HIST");

            builder.HasIndex(x => new { x.EventNo, x.BatId, x.ItemNo, x.LocId, x.TrxDat, x.SeqNo, x.SerSeqNo }).HasDatabaseName("PO_QRECV_HIST_SER_X1");
        }
    }

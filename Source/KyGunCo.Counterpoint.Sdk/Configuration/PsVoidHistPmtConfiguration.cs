...
{
    // PS_VOID_HIST_PMT
    public class PsVoidHistPmtConfiguration : IEntityTypeConfiguration<PsVoidHistPmt>
    {
        public void Configure(EntityTypeBuilder<PsVoidHistPmt> builder)
        {
            builder.ToTable("PS_VOID_HIST_PMT", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.PmtSeqNo }).HasName("PK_PS_VOID_HIST_PMT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsVoidHist).WithMany(b => b.PsVoidHistPmts).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_VOID_HIST_PMT_PS_VOID_HIST");

            builder.HasIndex(x => new { x.StrId, x.StaId, x.TktNo, x.PmtSeqNo }).HasDatabaseName("PS_VOID_HIST_PMT_X1");
        }
    }

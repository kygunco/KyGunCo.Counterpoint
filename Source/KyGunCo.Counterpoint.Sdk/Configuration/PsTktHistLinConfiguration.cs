...
{
    // PS_TKT_HIST_LIN
    public class PsTktHistLinConfiguration : IEntityTypeConfiguration<PsTktHistLin>
    {
        public void Configure(EntityTypeBuilder<PsTktHistLin> builder)
        {
            builder.ToTable("PS_TKT_HIST_LIN", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LinSeqNo }).HasName("PK_PS_TKT_HIST_LIN").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsTktHist).WithMany(b => b.PsTktHistLins).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_TKT_HIST_LIN_PS_TKT_HIST");

            builder.HasIndex(x => x.LinTyp).HasDatabaseName("IX_PS_TKT_HIST_LIN_LIN_TYP");
            builder.HasIndex(x => new { x.LinkLinGuid, x.LinTyp }).HasDatabaseName("IX_PS_TKT_HIST_LIN_LINK_LIN_GUID_LIN_TYP");
            builder.HasIndex(x => new { x.EventNo, x.StrId, x.StaId, x.TktNo, x.LinSeqNo }).HasDatabaseName("PS_TKT_HIST_LIN_X1");
            builder.HasIndex(x => x.ItemNo).HasDatabaseName("PS_TKT_HIST_LIN_X2");
            builder.HasIndex(x => x.LinGuid).HasDatabaseName("PS_TKT_HIST_LIN_X3").IsUnique();
            builder.HasIndex(x => new { x.LinTyp, x.SlsRep, x.BusDat }).HasDatabaseName("USER_IDX_PS_TKT_HIST_LIN");
            builder.HasIndex(x => new { x.DocId, x.ItemNo }).HasDatabaseName("USER_IDX_PS_TKT_HIST_LIN_1X");
            builder.HasIndex(x => x.DocId).HasDatabaseName("USER_IX_PS_TKT_HIST_LIN_DOC_ID");
            builder.HasIndex(x => x.DocId).HasDatabaseName("USER_IX_PS_TKT_HIST_LIN_DOC_ID_2");
            builder.HasIndex(x => x.DocId).HasDatabaseName("USER_IX_PS_TKT_HIST_LIN_DOC_ID_3");
        }
    }

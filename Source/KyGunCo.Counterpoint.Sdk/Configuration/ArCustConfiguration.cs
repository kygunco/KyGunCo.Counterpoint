...
{
    // AR_CUST
    public class ArCustConfiguration : IEntityTypeConfiguration<ArCust>
    {
        public void Configure(EntityTypeBuilder<ArCust> builder)
        {
            builder.ToTable("AR_CUST", "dbo");
            builder.HasKey(x => x.CustNo).HasName("PK_AR_CUST").IsClustered();
            ...
            builder.Property(x => x.RowTs).HasColumnName(@"ROW_TS").HasColumnType("timestamp").IsRequired(false).IsRowVersion();
            ...
            builder.Property(x => x.CustFstLstNam).HasColumnName(@"CUST_FST_LST_NAM").HasColumnType("varchar(42)").IsRequired(false).IsUnicode(false).HasMaxLength(42).ValueGeneratedOnAddOrUpdate();
            builder.Property(x => x.LstLoyEarnTktDt).HasColumnName(@"LST_LOY_EARN_TKT_DT").HasColumnType("datetime").IsRequired(false).ValueGeneratedOnAddOrUpdate();
            builder.Property(x => x.LstLoyRdmTktDt).HasColumnName(@"LST_LOY_RDM_TKT_DT").HasColumnType("datetime").IsRequired(false).ValueGeneratedOnAddOrUpdate();
            ...
            // Foreign keys
            builder.HasOne(a => a.ArCategCod).WithMany(b => b.ArCusts).HasForeignKey(c => c.CategCod).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_AR_CUST_AR_CATEG_COD");
            builder.HasOne(a => a.ArFchCod).WithMany(b => b.ArCusts).HasForeignKey(c => c.FchCod).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_AR_CUST_AR_FCH_COD");
            builder.HasOne(a => a.ArLoyPgm).WithMany(b => b.ArCusts).HasForeignKey(c => c.LoyPgmCod).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_AR_CUST_AR_LOY_PGM");
            builder.HasOne(a => a.ArTermsCod).WithMany(b => b.ArCusts).HasForeignKey(c => c.TermsCod).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_AR_CUST_AR_TERMS_COD");
            builder.HasOne(a => a.PsStr).WithMany(b => b.ArCusts).HasForeignKey(c => c.StrId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_AR_CUST_PS_STR");

            builder.HasIndex(x => new { x.CustNo, x.Nam, x.FstNam, x.LstNam, x.Phone1, x.Contct1, x.Contct2, x.EmailAdrs1, x.CategCod }).HasDatabaseName("AR_CUST_KEYWORDS");
            builder.HasIndex(x => x.RsStat).HasDatabaseName("AR_CUST_X_RS_STAT");
            builder.HasIndex(x => x.RsUtcDt).HasDatabaseName("AR_CUST_X_RS_UTC_DT");
            builder.HasIndex(x => new { x.BalMeth, x.CustNo }).HasDatabaseName("AR_CUST_X1");
            builder.HasIndex(x => new { x.TermsCod, x.CustNo }).HasDatabaseName("AR_CUST_X10");
            builder.HasIndex(x => new { x.EcommCustNo, x.CustNo }).HasDatabaseName("AR_CUST_X11");
            builder.HasIndex(x => new { x.LoyCardNo, x.CustNo }).HasDatabaseName("AR_CUST_X12");
            builder.HasIndex(x => new { x.FstNamUpr, x.CustNo }).HasDatabaseName("AR_CUST_X2");
            builder.HasIndex(x => new { x.LstNamUpr, x.CustNo }).HasDatabaseName("AR_CUST_X3");
            builder.HasIndex(x => new { x.NamUpr, x.CustNo }).HasDatabaseName("AR_CUST_X4");
            builder.HasIndex(x => new { x.Phone1, x.CustNo }).HasDatabaseName("AR_CUST_X5");
            builder.HasIndex(x => new { x.CustTyp, x.CustNo }).HasDatabaseName("AR_CUST_X6");
            builder.HasIndex(x => new { x.ZipCod, x.CustNo }).HasDatabaseName("AR_CUST_X7");
            builder.HasIndex(x => new { x.StrId, x.CustNo }).HasDatabaseName("AR_CUST_X8");
            builder.HasIndex(x => new { x.CategCod, x.CustNo }).HasDatabaseName("AR_CUST_X9");
            builder.HasIndex(x => new { x.State, x.UserDlNo }).HasDatabaseName("USER_IX_AR_CUST_STATE_USER_DL_NO");
        }
    }

...
{
    // PS_VOID_HIST_LIN_CELL_EXT
    public class PsVoidHistLinCellExtConfiguration : IEntityTypeConfiguration<PsVoidHistLinCellExt>
    {
        public void Configure(EntityTypeBuilder<PsVoidHistLinCellExt> builder)
        {
            builder.ToTable("PS_VOID_HIST_LIN_CELL_EXT", "dbo");
            builder.HasKey(x => new { x.BusDatExt, x.DocIdExt, x.LinSeqNoExt, x.Dim1UprExt, x.Dim2UprExt, x.Dim3UprExt }).HasName("PK_PS_VOID_HIST_LIN_CELL_EXT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsVoidHistLinCell).WithOne(b => b.PsVoidHistLinCellExt).HasForeignKey<PsVoidHistLinCellExt>(c => new { c.BusDatExt, c.DocIdExt, c.LinSeqNoExt, c.Dim1UprExt, c.Dim2UprExt, c.Dim3UprExt }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("PS_VOID_HIST_LIN_CELL_EXT_PS_VOID_HIST_LIN_CELL");
        }
    }

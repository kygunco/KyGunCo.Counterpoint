...
{
    // PO_RECVR_NOTE
    public class PoRecvrNoteConfiguration : IEntityTypeConfiguration<PoRecvrNote>
    {
        public void Configure(EntityTypeBuilder<PoRecvrNote> builder)
        {
            builder.ToTable("PO_RECVR_NOTE", "dbo");
            builder.HasKey(x => new { x.RecvrNo, x.NoteId }).HasName("PK_PO_RECVR_NOTE").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PoRecvrHdr).WithMany(b => b.PoRecvrNotes).HasForeignKey(c => c.RecvrNo).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_RECVR_NOTE_PO_RECVR_HDR");

            builder.HasIndex(x => new { x.RecvrNo, x.NoteDat, x.NoteId }).HasDatabaseName("PO_RECVR_NOTE_X1");
        }
    }

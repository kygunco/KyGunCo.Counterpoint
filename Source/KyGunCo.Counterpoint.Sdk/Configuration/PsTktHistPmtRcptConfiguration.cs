...
{
    // PS_TKT_HIST_PMT_RCPT
    public class PsTktHistPmtRcptConfiguration : IEntityTypeConfiguration<PsTktHistPmtRcpt>
    {
        public void Configure(EntityTypeBuilder<PsTktHistPmtRcpt> builder)
        {
            builder.ToTable("PS_TKT_HIST_PMT_RCPT", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.PmtSeqNo }).HasName("PK_PS_TKT_HIST_PMT_RCPT").IsClustered();
            ...
            builder.Property(x => x.OfflineAuthCod).HasColumnName(@"OFFLINE_AUTH_COD").HasColumnType("varchar(10)").IsRequired(false).IsUnicode(false).HasMaxLength(10);
            builder.Property(x => x.OfflineTransStatus).HasColumnName(@"OFFLINE_TRANS_STATUS").HasColumnType("varchar(1)").IsRequired(false).IsUnicode(false).HasMaxLength(1);
            builder.Property(x => x.OfflineLstMaintDt).HasColumnName(@"OFFLINE_LST_MAINT_DT").HasColumnType("datetime").IsRequired(false);
            ...
            // Foreign keys
            builder.HasOne(a => a.PsTktHistPmt).WithOne(b => b.PsTktHistPmtRcpt).HasForeignKey<PsTktHistPmtRcpt>(c => new { c.BusDat, c.DocId, c.PmtSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_TKT_HIST_PMT_RCPT_PS_TKT_HIST_PMT");
        }
    }

...
{
    // PS_LWY_HIST_PMT_CR_CARD
    public class PsLwyHistPmtCrCardConfiguration : IEntityTypeConfiguration<PsLwyHistPmtCrCard>
    {
        public void Configure(EntityTypeBuilder<PsLwyHistPmtCrCard> builder)
        {
            builder.ToTable("PS_LWY_HIST_PMT_CR_CARD", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.PmtSeqNo }).HasName("PK_PS_LWY_HIST_PMT_CR_CARD").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsLwyHistPmt).WithOne(b => b.PsLwyHistPmtCrCard).HasForeignKey<PsLwyHistPmtCrCard>(c => new { c.BusDat, c.DocId, c.PmtSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_LWY_HIST_PMT_CR_CARD_PS_LWY_HIST_PMT");
        }
    }

...
{
    // PS_ORD_HIST
    public class PsOrdHistConfiguration : IEntityTypeConfiguration<PsOrdHist>
    {
        public void Configure(EntityTypeBuilder<PsOrdHist> builder)
        {
            builder.ToTable("PS_ORD_HIST", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId }).HasName("PK_PS_ORD_HIST").IsClustered();
            ...
            builder.HasIndex(x => new { x.EventNo, x.StrId, x.StaId, x.TktNo }).HasDatabaseName("PS_ORD_HIST_X1");
            builder.HasIndex(x => new { x.StrId, x.BusDat }).HasDatabaseName("PS_ORD_HIST_X2");
            builder.HasIndex(x => x.TktNo).HasDatabaseName("PS_ORD_HIST_X3");
            builder.HasIndex(x => x.CustNo).HasDatabaseName("PS_ORD_HIST_X4");
            builder.HasIndex(x => x.DocId).HasDatabaseName("USER_IX_PS_ORD_HIST_DOC_ID");
        }
    }

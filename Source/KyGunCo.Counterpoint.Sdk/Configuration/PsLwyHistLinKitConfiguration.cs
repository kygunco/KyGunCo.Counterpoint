...
{
    // PS_LWY_HIST_LIN_KIT
    public class PsLwyHistLinKitConfiguration : IEntityTypeConfiguration<PsLwyHistLinKit>
    {
        public void Configure(EntityTypeBuilder<PsLwyHistLinKit> builder)
        {
            builder.ToTable("PS_LWY_HIST_LIN_KIT", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LinSeqNo }).HasName("PK_PS_LWY_HIST_LIN_KIT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsLwyHistLin).WithOne(b => b.PsLwyHistLinKit).HasForeignKey<PsLwyHistLinKit>(c => new { c.BusDat, c.DocId, c.LinSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_LWY_HIST_LIN_KIT_PS_LWY_HIST_LIN");
        }
    }

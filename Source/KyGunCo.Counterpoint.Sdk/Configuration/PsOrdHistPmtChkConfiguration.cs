...
{
    // PS_ORD_HIST_PMT_CHK
    public class PsOrdHistPmtChkConfiguration : IEntityTypeConfiguration<PsOrdHistPmtChk>
    {
        public void Configure(EntityTypeBuilder<PsOrdHistPmtChk> builder)
        {
            builder.ToTable("PS_ORD_HIST_PMT_CHK", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.PmtSeqNo }).HasName("PK_PS_ORD_HIST_PMT_CHK").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsOrdHistPmt).WithOne(b => b.PsOrdHistPmtChk).HasForeignKey<PsOrdHistPmtChk>(c => new { c.BusDat, c.DocId, c.PmtSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_ORD_HIST_PMT_CHK_PS_ORD_HIST_PMT");
        }
    }

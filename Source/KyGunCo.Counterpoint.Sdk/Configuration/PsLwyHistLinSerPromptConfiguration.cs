...
{
    // PS_LWY_HIST_LIN_SER_PROMPT
    public class PsLwyHistLinSerPromptConfiguration : IEntityTypeConfiguration<PsLwyHistLinSerPrompt>
    {
        public void Configure(EntityTypeBuilder<PsLwyHistLinSerPrompt> builder)
        {
            builder.ToTable("PS_LWY_HIST_LIN_SER_PROMPT", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LinSeqNo, x.SerNo, x.PromptSeqNo }).HasName("PK_PS_LWY_HIST_LIN_SER_PROMPT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsLwyHistLinSer).WithMany(b => b.PsLwyHistLinSerPrompts).HasForeignKey(c => new { c.BusDat, c.DocId, c.LinSeqNo, c.SerNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_LWY_HIST_LIN_SER_PROMPT_PS_LWY_HIST_LIN_SER");
        }
    }

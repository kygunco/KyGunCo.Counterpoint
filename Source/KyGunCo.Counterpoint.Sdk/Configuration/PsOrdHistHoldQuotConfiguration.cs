...
{
    // PS_ORD_HIST_HOLD_QUOT
    public class PsOrdHistHoldQuotConfiguration : IEntityTypeConfiguration<PsOrdHistHoldQuot>
    {
        public void Configure(EntityTypeBuilder<PsOrdHistHoldQuot> builder)
        {
            builder.ToTable("PS_ORD_HIST_HOLD_QUOT", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId }).HasName("PK_PS_ORD_HIST_HOLD_QUOT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsOrdHist).WithOne(b => b.PsOrdHistHoldQuot).HasForeignKey<PsOrdHistHoldQuot>(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_ORD_HIST_HOLD_QUOT_PS_ORD_HIST");
        }
    }

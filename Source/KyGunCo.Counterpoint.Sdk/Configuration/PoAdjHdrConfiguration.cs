...
{
    // PO_ADJ_HDR
    public class PoAdjHdrConfiguration : IEntityTypeConfiguration<PoAdjHdr>
    {
        public void Configure(EntityTypeBuilder<PoAdjHdr> builder)
        {
            builder.ToTable("PO_ADJ_HDR", "dbo");
            builder.HasKey(x => x.RecvrNo).HasName("PK_PO_ADJ_HDR").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ArTaxCod).WithMany(b => b.PoAdjHdrs).HasForeignKey(c => c.TaxCod).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_ADJ_HDR_AR_TAX_COD");
            builder.HasOne(a => a.ImLoc).WithMany(b => b.PoAdjHdrs).HasForeignKey(c => c.RecvrLocId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_ADJ_HDR_IM_LOC");

            builder.HasIndex(x => new { x.VendNo, x.RecvrNo }).HasDatabaseName("PO_ADJ_HDR_X1");
            builder.HasIndex(x => new { x.RecvrLocId, x.RecvrNo }).HasDatabaseName("PO_ADJ_HDR_X2");
            builder.HasIndex(x => new { x.VendNamUpr, x.RecvrNo }).HasDatabaseName("PO_ADJ_HDR_X3");
            builder.HasIndex(x => new { x.VendFstNamUpr, x.RecvrNo }).HasDatabaseName("PO_ADJ_HDR_X4");
            builder.HasIndex(x => new { x.VendLstNamUpr, x.RecvrNo }).HasDatabaseName("PO_ADJ_HDR_X5");
        }
    }

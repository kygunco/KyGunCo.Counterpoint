...
{
    // PO_RECVR_HIST_TAX
    public class PoRecvrHistTaxConfiguration : IEntityTypeConfiguration<PoRecvrHistTax>
    {
        public void Configure(EntityTypeBuilder<PoRecvrHistTax> builder)
        {
            builder.ToTable("PO_RECVR_HIST_TAX", "dbo");
            builder.HasKey(x => new { x.RecvrNo, x.AuthCod, x.RulCod, x.TaxDetlTyp }).HasName("PK_PO_RECVR_HIST_TAX").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PoRecvrHist).WithMany(b => b.PoRecvrHistTaxes).HasForeignKey(c => c.RecvrNo).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_RECVR_HIST_TAX_PO_RECVR_HIST");
        }
    }

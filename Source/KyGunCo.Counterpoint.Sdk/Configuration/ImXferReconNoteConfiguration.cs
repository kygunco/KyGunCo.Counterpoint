...
{
    // IM_XFER_RECON_NOTE
    public class ImXferReconNoteConfiguration : IEntityTypeConfiguration<ImXferReconNote>
    {
        public void Configure(EntityTypeBuilder<ImXferReconNote> builder)
        {
            builder.ToTable("IM_XFER_RECON_NOTE", "dbo");
            builder.HasKey(x => new { x.XferNo, x.NoteId }).HasName("PK_IM_XFER_RECON_NOTE").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ImXferReconHdr).WithMany(b => b.ImXferReconNotes).HasForeignKey(c => c.XferNo).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_XFER_RECON_NOTE_IM_XFER_RECON_HDR");

            builder.HasIndex(x => new { x.XferNo, x.NoteDat, x.NoteId }).HasDatabaseName("IM_XFER_RECON_NOTE_X1");
        }
    }

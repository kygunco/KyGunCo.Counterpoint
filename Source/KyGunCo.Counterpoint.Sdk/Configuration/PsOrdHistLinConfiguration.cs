...
{
    // PS_ORD_HIST_LIN
    public class PsOrdHistLinConfiguration : IEntityTypeConfiguration<PsOrdHistLin>
    {
        public void Configure(EntityTypeBuilder<PsOrdHistLin> builder)
        {
            builder.ToTable("PS_ORD_HIST_LIN", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LinSeqNo }).HasName("PK_PS_ORD_HIST_LIN").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsOrdHist).WithMany(b => b.PsOrdHistLins).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_ORD_HIST_LIN_PS_ORD_HIST");

            builder.HasIndex(x => new { x.EventNo, x.StrId, x.StaId, x.TktNo, x.LinSeqNo }).HasDatabaseName("PS_ORD_HIST_LIN_X1");
            builder.HasIndex(x => x.LinGuid).HasDatabaseName("PS_ORD_HIST_LIN_X2").IsUnique();
        }
    }

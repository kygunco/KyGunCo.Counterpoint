...
{
    // PS_VOID_HIST_PMT_CR_CARD
    public class PsVoidHistPmtCrCardConfiguration : IEntityTypeConfiguration<PsVoidHistPmtCrCard>
    {
        public void Configure(EntityTypeBuilder<PsVoidHistPmtCrCard> builder)
        {
            builder.ToTable("PS_VOID_HIST_PMT_CR_CARD", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.PmtSeqNo }).HasName("PK_PS_VOID_HIST_PMT_CR_CARD").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsVoidHistPmt).WithOne(b => b.PsVoidHistPmtCrCard).HasForeignKey<PsVoidHistPmtCrCard>(c => new { c.BusDat, c.DocId, c.PmtSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_VOID_HIST_PMT_CR_CARD_PS_VOID_HIST_PMT");
        }
    }

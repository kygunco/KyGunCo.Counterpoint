...
{
    // PS_DOC_SVC
    public class PsDocSvcConfiguration : IEntityTypeConfiguration<PsDocSvc>
    {
        public void Configure(EntityTypeBuilder<PsDocSvc> builder)
        {
            builder.ToTable("PS_DOC_SVC", "dbo");
            builder.HasKey(x => new { x.DocId, x.SvcSeqNo }).HasName("PK_PS_DOC_SVC").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsDocHdr).WithMany(b => b.PsDocSvcs).HasForeignKey(c => c.DocId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_DOC_SVC_PS_DOC_HDR");
        }
    }

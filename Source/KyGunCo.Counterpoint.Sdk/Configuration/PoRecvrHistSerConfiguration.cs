...
{
    // PO_RECVR_HIST_SER
    public class PoRecvrHistSerConfiguration : IEntityTypeConfiguration<PoRecvrHistSer>
    {
        public void Configure(EntityTypeBuilder<PoRecvrHistSer> builder)
        {
            builder.ToTable("PO_RECVR_HIST_SER", "dbo");
            builder.HasKey(x => new { x.RecvrNo, x.SeqNo, x.SerNo }).HasName("PK_PO_RECVR_HIST_SER").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PoRecvrHistLin).WithMany(b => b.PoRecvrHistSers).HasForeignKey(c => new { c.RecvrNo, c.SeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_RECVR_HIST_SER_PO_RECVR_HIST_LIN");

            builder.HasIndex(x => new { x.RecvrNo, x.SeqNo, x.SerSeqNo }).HasDatabaseName("PO_RECVR_HIST_SER_X1");
        }
    }

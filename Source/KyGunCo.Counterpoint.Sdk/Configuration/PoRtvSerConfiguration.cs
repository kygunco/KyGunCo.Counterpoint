...
{
    // PO_RTV_SER
    public class PoRtvSerConfiguration : IEntityTypeConfiguration<PoRtvSer>
    {
        public void Configure(EntityTypeBuilder<PoRtvSer> builder)
        {
            builder.ToTable("PO_RTV_SER", "dbo");
            builder.HasKey(x => new { x.RtvNo, x.SeqNo, x.SerNo }).HasName("PK_PO_RTV_SER").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PoRtvLin).WithMany(b => b.PoRtvSers).HasForeignKey(c => new { c.RtvNo, c.SeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_RTV_SER_PO_RTV_LIN");

            builder.HasIndex(x => new { x.RtvNo, x.SeqNo, x.SerSeqNo }).HasDatabaseName("PO_RTV_SER_X1");
        }
    }

...
{
    // PS_TKT_HIST_LIN_PROMPT
    public class PsTktHistLinPromptConfiguration : IEntityTypeConfiguration<PsTktHistLinPrompt>
    {
        public void Configure(EntityTypeBuilder<PsTktHistLinPrompt> builder)
        {
            builder.ToTable("PS_TKT_HIST_LIN_PROMPT", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LinSeqNo, x.PromptSeqNo }).HasName("PK_PS_TKT_HIST_LIN_PROMPT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsTktHistLin).WithMany(b => b.PsTktHistLinPrompts).HasForeignKey(c => new { c.BusDat, c.DocId, c.LinSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_TKT_HIST_LIN_PROMPT_PS_TKT_HIST_LIN");
        }
    }

...
{
    // PS_TKT_HIST_LIN_KIT
    public class PsTktHistLinKitConfiguration : IEntityTypeConfiguration<PsTktHistLinKit>
    {
        public void Configure(EntityTypeBuilder<PsTktHistLinKit> builder)
        {
            builder.ToTable("PS_TKT_HIST_LIN_KIT", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LinSeqNo }).HasName("PK_PS_TKT_HIST_LIN_KIT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsTktHistLin).WithOne(b => b.PsTktHistLinKit).HasForeignKey<PsTktHistLinKit>(c => new { c.BusDat, c.DocId, c.LinSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_TKT_HIST_LIN_KIT_PS_TKT_HIST_LIN");
        }
    }

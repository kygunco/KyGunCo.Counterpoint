...
{
    // AR_CASH_HIST
    public class ArCashHistConfiguration : IEntityTypeConfiguration<ArCashHist>
    {
        public void Configure(EntityTypeBuilder<ArCashHist> builder)
        {
            builder.ToTable("AR_CASH_HIST", "dbo");
            builder.HasKey(x => new { x.EventNo, x.CustNo, x.DocNo }).HasName("PK_AR_CASH_HIST").IsClustered();
            ...
            builder.Property(x => x.RvrsdByEventNo).HasColumnName(@"RVRSD_BY_EVENT_NO").HasColumnType("varchar(15)").IsRequired(false).IsUnicode(false).HasMaxLength(15);
            builder.Property(x => x.RvrsdByDocNo).HasColumnName(@"RVRSD_BY_DOC_NO").HasColumnType("varchar(15)").IsRequired(false).IsUnicode(false).HasMaxLength(15);

            // Foreign keys
            builder.HasOne(a => a.PsStrHist).WithMany(b => b.ArCashHists).HasForeignKey(c => c.CustStrId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_AR_CASH_HIST_PS_STR_HIST");

            builder.HasIndex(x => new { x.EventNo, x.EntrySeqNo, x.CustNo, x.DocNo }).HasDatabaseName("AR_CASH_HIST_X1");
            builder.HasIndex(x => new { x.CustNo, x.EventNo, x.DocNo }).HasDatabaseName("AR_CASH_HIST_X2");
        }
    }

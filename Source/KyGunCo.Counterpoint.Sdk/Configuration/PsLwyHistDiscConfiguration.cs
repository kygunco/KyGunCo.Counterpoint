...
{
    // PS_LWY_HIST_DISC
    public class PsLwyHistDiscConfiguration : IEntityTypeConfiguration<PsLwyHistDisc>
    {
        public void Configure(EntityTypeBuilder<PsLwyHistDisc> builder)
        {
            builder.ToTable("PS_LWY_HIST_DISC", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.DiscSeqNo }).HasName("PK_PS_LWY_HIST_DISC").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsLwyHist).WithMany(b => b.PsLwyHistDiscs).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_LWY_HIST_DISC_PS_LWY_HIST");
            builder.HasOne(a => a.PsLwyHistLin).WithMany(b => b.PsLwyHistDiscs).HasForeignKey(c => new { c.BusDat, c.DocId, c.LinSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_LWY_HIST_DISC_PS_LWY_HIST_LIN");
        }
    }

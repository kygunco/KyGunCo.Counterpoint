...
{
    // PO_RTV_HIST_TAX
    public class PoRtvHistTaxConfiguration : IEntityTypeConfiguration<PoRtvHistTax>
    {
        public void Configure(EntityTypeBuilder<PoRtvHistTax> builder)
        {
            builder.ToTable("PO_RTV_HIST_TAX", "dbo");
            builder.HasKey(x => new { x.RtvNo, x.AuthCod, x.RulCod }).HasName("PK_PO_RTV_HIST_TAX").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PoRtvHist).WithMany(b => b.PoRtvHistTaxes).HasForeignKey(c => c.RtvNo).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_RTV_HIST_TAX_PO_RTV_HIST");
        }
    }

...
{
    // PO_RECVR_SER
    public class PoRecvrSerConfiguration : IEntityTypeConfiguration<PoRecvrSer>
    {
        public void Configure(EntityTypeBuilder<PoRecvrSer> builder)
        {
            builder.ToTable("PO_RECVR_SER", "dbo");
            builder.HasKey(x => new { x.RecvrNo, x.SeqNo, x.SerNo }).HasName("PK_PO_RECVR_SER").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PoRecvrLin).WithMany(b => b.PoRecvrSers).HasForeignKey(c => new { c.RecvrNo, c.SeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_RECVR_SER_PO_RECVR_LIN");

            builder.HasIndex(x => new { x.RecvrNo, x.SeqNo, x.SerSeqNo }).HasDatabaseName("PO_RECVR_SER_X1");
        }
    }

...
{
    // IM_ADJ_TRX_SER
    public class ImAdjTrxSerConfiguration : IEntityTypeConfiguration<ImAdjTrxSer>
    {
        public void Configure(EntityTypeBuilder<ImAdjTrxSer> builder)
        {
            builder.ToTable("IM_ADJ_TRX_SER", "dbo");
            builder.HasKey(x => new { x.BatId, x.ItemNo, x.LocId, x.TrxDat, x.SeqNo, x.SerNo }).HasName("PK_IM_ADJ_TRX_SER").IsClustered();
            ...
            builder.Property(x => x.SerIsNew).HasColumnName(@"SER_IS_NEW").HasColumnType("varchar(1)").IsRequired().IsUnicode(false).HasMaxLength(1);

            // Foreign keys
            builder.HasOne(a => a.ImAdjTrx).WithMany(b => b.ImAdjTrxSers).HasForeignKey(c => new { c.BatId, c.ItemNo, c.LocId, c.TrxDat, c.SeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_ADJ_TRX_SER_IM_ADJ_TRX");

            builder.HasIndex(x => new { x.BatId, x.ItemNo, x.LocId, x.TrxDat, x.SeqNo, x.SerSeqNo }).HasDatabaseName("IM_ADJ_TRX_SER_X1");
        }
    }

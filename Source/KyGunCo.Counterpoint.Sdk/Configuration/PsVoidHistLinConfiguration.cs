...
{
    // PS_VOID_HIST_LIN
    public class PsVoidHistLinConfiguration : IEntityTypeConfiguration<PsVoidHistLin>
    {
        public void Configure(EntityTypeBuilder<PsVoidHistLin> builder)
        {
            builder.ToTable("PS_VOID_HIST_LIN", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LinSeqNo }).HasName("PK_PS_VOID_HIST_LIN").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsVoidHist).WithMany(b => b.PsVoidHistLins).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_VOID_HIST_LIN_PS_VOID_HIST");

            builder.HasIndex(x => new { x.EventNo, x.StrId, x.StaId, x.TktNo, x.LinSeqNo }).HasDatabaseName("PS_VOID_HIST_LIN_X1");
            builder.HasIndex(x => x.LinGuid).HasDatabaseName("PS_VOID_HIST_LIN_X2").IsUnique();
        }
    }

...
{
    // PO_RTV_HIST_CELL
    public class PoRtvHistCellConfiguration : IEntityTypeConfiguration<PoRtvHistCell>
    {
        public void Configure(EntityTypeBuilder<PoRtvHistCell> builder)
        {
            builder.ToTable("PO_RTV_HIST_CELL", "dbo");
            builder.HasKey(x => new { x.RtvNo, x.SeqNo, x.Dim1Upr, x.Dim2Upr, x.Dim3Upr }).HasName("PK_PO_RTV_HIST_CELL").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PoRtvHistLin).WithMany(b => b.PoRtvHistCells).HasForeignKey(c => new { c.RtvNo, c.SeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PO_RTV_HIST_CELL_PO_RTV_HIST_LIN");
        }
    }

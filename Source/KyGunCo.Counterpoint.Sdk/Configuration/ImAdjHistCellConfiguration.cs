...
{
    // IM_ADJ_HIST_CELL
    public class ImAdjHistCellConfiguration : IEntityTypeConfiguration<ImAdjHistCell>
    {
        public void Configure(EntityTypeBuilder<ImAdjHistCell> builder)
        {
            builder.ToTable("IM_ADJ_HIST_CELL", "dbo");
            builder.HasKey(x => new { x.EventNo, x.BatId, x.ItemNo, x.LocId, x.TrxDat, x.SeqNo, x.Dim1Upr, x.Dim2Upr, x.Dim3Upr }).HasName("PK_IM_ADJ_HIST_CELL").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ImAdjHist).WithMany(b => b.ImAdjHistCells).HasForeignKey(c => new { c.EventNo, c.BatId, c.ItemNo, c.LocId, c.TrxDat, c.SeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_ADJ_HIST_CELL_IM_ADJ_HIST");
        }
    }

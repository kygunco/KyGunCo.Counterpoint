...
{
    // PS_LWY_HIST_NOTE
    public class PsLwyHistNoteConfiguration : IEntityTypeConfiguration<PsLwyHistNote>
    {
        public void Configure(EntityTypeBuilder<PsLwyHistNote> builder)
        {
            builder.ToTable("PS_LWY_HIST_NOTE", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.NoteSeqNo }).HasName("PK_PS_LWY_HIST_NOTE").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsLwyHist).WithMany(b => b.PsLwyHistNotes).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_LWY_HIST_NOTE_PS_LWY_HIST");
        }
    }

...
{
    // PS_TKT_HIST_PMT_CR_CARD
    public class PsTktHistPmtCrCardConfiguration : IEntityTypeConfiguration<PsTktHistPmtCrCard>
    {
        public void Configure(EntityTypeBuilder<PsTktHistPmtCrCard> builder)
        {
            builder.ToTable("PS_TKT_HIST_PMT_CR_CARD", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.PmtSeqNo }).HasName("PK_PS_TKT_HIST_PMT_CR_CARD").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsTktHistPmt).WithOne(b => b.PsTktHistPmtCrCard).HasForeignKey<PsTktHistPmtCrCard>(c => new { c.BusDat, c.DocId, c.PmtSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_TKT_HIST_PMT_CR_CARD_PS_TKT_HIST_PMT");

            builder.HasIndex(x => x.CrCardNoMsk).HasDatabaseName("PS_TKT_HIST_PMT_CR_CARD_X1");
        }
    }

...
{
    // PS_DOC_AUDIT_LOG
    public class PsDocAuditLogConfiguration : IEntityTypeConfiguration<PsDocAuditLog>
    {
        public void Configure(EntityTypeBuilder<PsDocAuditLog> builder)
        {
            builder.ToTable("PS_DOC_AUDIT_LOG", "dbo");
            builder.HasKey(x => new { x.DocId, x.LogSeqNo }).HasName("PK_PS_DOC_AUDIT_LOG").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsDocHdr).WithMany(b => b.PsDocAuditLogs).HasForeignKey(c => c.DocId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_DOC_AUDIT_LOG_PS_DOC_HDR");
        }
    }

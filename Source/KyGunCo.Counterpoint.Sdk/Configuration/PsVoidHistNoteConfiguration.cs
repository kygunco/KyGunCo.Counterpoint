...
{
    // PS_VOID_HIST_NOTE
    public class PsVoidHistNoteConfiguration : IEntityTypeConfiguration<PsVoidHistNote>
    {
        public void Configure(EntityTypeBuilder<PsVoidHistNote> builder)
        {
            builder.ToTable("PS_VOID_HIST_NOTE", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.NoteSeqNo }).HasName("PK_PS_VOID_HIST_NOTE").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsVoidHist).WithMany(b => b.PsVoidHistNotes).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_VOID_HIST_NOTE_PS_VOID_HIST");
        }
    }

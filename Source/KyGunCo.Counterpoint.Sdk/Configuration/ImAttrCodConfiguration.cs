...
{
    // IM_ATTR_COD
    public class ImAttrCodConfiguration : IEntityTypeConfiguration<ImAttrCod>
    {
        public void Configure(EntityTypeBuilder<ImAttrCod> builder)
        {
            builder.ToTable("IM_ATTR_COD", "dbo");
            builder.HasKey(x => x.AttrCod).HasName("PK_IM_ATTR_COD").IsClustered();

            builder.Property(x => x.AttrCod).HasColumnName(@"ATTR_COD").HasColumnType("varchar(10)").IsRequired().IsUnicode(false).HasMaxLength(10).ValueGeneratedNever();
            ...
            builder.Property(x => x.Val6).HasColumnName(@"VAL_6").HasColumnType("varchar(1)").IsRequired().IsUnicode(false).HasMaxLength(1);
            ...
            builder.HasIndex(x => x.RsStat).HasDatabaseName("IM_ATTR_COD_X_RS_STAT");
            builder.HasIndex(x => x.RsUtcDt).HasDatabaseName("IM_ATTR_COD_X_RS_UTC_DT");
            builder.HasIndex(x => new { x.DescrUpr, x.AttrCod }).HasDatabaseName("IM_ATTR_COD_X1");
        }
    }

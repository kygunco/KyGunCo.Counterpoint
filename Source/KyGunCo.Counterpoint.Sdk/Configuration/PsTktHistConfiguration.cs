...
{
    // PS_TKT_HIST
    public class PsTktHistConfiguration : IEntityTypeConfiguration<PsTktHist>
    {
        public void Configure(EntityTypeBuilder<PsTktHist> builder)
        {
            builder.ToTable("PS_TKT_HIST", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId }).HasName("PK_PS_TKT_HIST").IsClustered();
            ...
            builder.Property(x => x.AggStat).HasColumnName(@"AGG_STAT").HasColumnType("varchar(1)").IsRequired(false).IsUnicode(false).HasMaxLength(1);
            builder.Property(x => x.ReceiptNum).HasColumnName(@"RECEIPT_NUM").HasColumnType("int").IsRequired(false);
            builder.Property(x => x.TotTipAmt).HasColumnName(@"TOT_TIP_AMT").HasColumnType("decimal(15,2)").IsRequired();

            builder.HasIndex(x => new { x.EventNo, x.StrId, x.StaId, x.TktNo }).HasDatabaseName("PS_TKT_HIST_X1");
            builder.HasIndex(x => new { x.StrId, x.BusDat }).HasDatabaseName("PS_TKT_HIST_X2");
            builder.HasIndex(x => x.TktNo).HasDatabaseName("PS_TKT_HIST_X3");
            builder.HasIndex(x => x.CustNo).HasDatabaseName("PS_TKT_HIST_X4");
            builder.HasIndex(x => new { x.DocId, x.TktDt }).HasDatabaseName("PS_TKT_HIST_X5");
            builder.HasIndex(x => x.CustNo).HasDatabaseName("USER_IDX_PS_TKT_HIST_1X");
            builder.HasIndex(x => x.EventNo).HasDatabaseName("USER_IX_PS_TKT_HIST_EVENT_NO");
        }
    }

...
{
    // PS_VOID_HIST_PROF
    public class PsVoidHistProfConfiguration : IEntityTypeConfiguration<PsVoidHistProf>
    {
        public void Configure(EntityTypeBuilder<PsVoidHistProf> builder)
        {
            builder.ToTable("PS_VOID_HIST_PROF", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId }).HasName("PK_PS_VOID_HIST_PROF").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsVoidHist).WithOne(b => b.PsVoidHistProf).HasForeignKey<PsVoidHistProf>(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_VOID_HIST_PROF_PS_VOID_HIST");
        }
    }

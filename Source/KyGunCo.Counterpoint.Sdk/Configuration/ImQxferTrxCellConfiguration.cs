...
{
    // IM_QXFER_TRX_CELL
    public class ImQxferTrxCellConfiguration : IEntityTypeConfiguration<ImQxferTrxCell>
    {
        public void Configure(EntityTypeBuilder<ImQxferTrxCell> builder)
        {
            builder.ToTable("IM_QXFER_TRX_CELL", "dbo");
            builder.HasKey(x => new { x.BatId, x.ItemNo, x.FromLocId, x.ToLocId, x.TrxDat, x.SeqNo, x.Dim1Upr, x.Dim2Upr, x.Dim3Upr }).HasName("PK_IM_QXFER_TRX_CELL").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.ImQxferTrx).WithMany(b => b.ImQxferTrxCells).HasForeignKey(c => new { c.BatId, c.ItemNo, c.FromLocId, c.ToLocId, c.TrxDat, c.SeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IM_QXFER_TRX_CELL_IM_QXFER_TRX");
        }
    }

...
{
    // PS_TKT_HIST_LIN_PO
    public class PsTktHistLinPoConfiguration : IEntityTypeConfiguration<PsTktHistLinPo>
    {
        public void Configure(EntityTypeBuilder<PsTktHistLinPo> builder)
        {
            builder.ToTable("PS_TKT_HIST_LIN_PO", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.LinSeqNo }).HasName("PK_PS_TKT_HIST_LIN_PO").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsTktHistLin).WithOne(b => b.PsTktHistLinPo).HasForeignKey<PsTktHistLinPo>(c => new { c.BusDat, c.DocId, c.LinSeqNo }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_TKT_HIST_LIN_PO_PS_TKT_HIST_LIN");
        }
    }

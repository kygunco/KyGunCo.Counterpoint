...
{
    // PS_LWY_HIST_TAX
    public class PsLwyHistTaxConfiguration : IEntityTypeConfiguration<PsLwyHistTax>
    {
        public void Configure(EntityTypeBuilder<PsLwyHistTax> builder)
        {
            builder.ToTable("PS_LWY_HIST_TAX", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.AuthCod, x.RulCod }).HasName("PK_PS_LWY_HIST_TAX").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsLwyHist).WithMany(b => b.PsLwyHistTaxes).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_LWY_HIST_TAX_PS_LWY_HIST");
        }
    }

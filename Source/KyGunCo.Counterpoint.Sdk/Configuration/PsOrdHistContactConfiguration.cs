...
{
    // PS_ORD_HIST_CONTACT
    public class PsOrdHistContactConfiguration : IEntityTypeConfiguration<PsOrdHistContact>
    {
        public void Configure(EntityTypeBuilder<PsOrdHistContact> builder)
        {
            builder.ToTable("PS_ORD_HIST_CONTACT", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.ContactId }).HasName("PK_PS_ORD_HIST_CONTACT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsOrdHist).WithMany(b => b.PsOrdHistContacts).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_ORD_HIST_CONTACT_PS_ORD_HIST");

            builder.HasIndex(x => new { x.LstNam, x.FstNam }).HasDatabaseName("PS_ORD_HIST_CONTACT_X1");
            builder.HasIndex(x => x.Nam).HasDatabaseName("PS_ORD_HIST_CONTACT_X2");
        }
    }

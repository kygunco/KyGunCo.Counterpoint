...
{
    // PS_TKT_HIST_TAX
    public class PsTktHistTaxConfiguration : IEntityTypeConfiguration<PsTktHistTax>
    {
        public void Configure(EntityTypeBuilder<PsTktHistTax> builder)
        {
            builder.ToTable("PS_TKT_HIST_TAX", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.AuthCod, x.RulCod }).HasName("PK_PS_TKT_HIST_TAX").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsTktHist).WithMany(b => b.PsTktHistTaxes).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_TKT_HIST_TAX_PS_TKT_HIST");
        }
    }

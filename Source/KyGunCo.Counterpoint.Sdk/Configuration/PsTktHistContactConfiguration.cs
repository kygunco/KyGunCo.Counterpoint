...
{
    // PS_TKT_HIST_CONTACT
    public class PsTktHistContactConfiguration : IEntityTypeConfiguration<PsTktHistContact>
    {
        public void Configure(EntityTypeBuilder<PsTktHistContact> builder)
        {
            builder.ToTable("PS_TKT_HIST_CONTACT", "dbo");
            builder.HasKey(x => new { x.BusDat, x.DocId, x.ContactId }).HasName("PK_PS_TKT_HIST_CONTACT").IsClustered();
            ...
            // Foreign keys
            builder.HasOne(a => a.PsTktHist).WithMany(b => b.PsTktHistContacts).HasForeignKey(c => new { c.BusDat, c.DocId }).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_PS_TKT_HIST_CONTACT_PS_TKT_HIST");

            builder.HasIndex(x => new { x.LstNam, x.FstNam }).HasDatabaseName("PS_TKT_HIST_CONTACT_X1");
            builder.HasIndex(x => x.Nam).HasDatabaseName("PS_TKT_HIST_CONTACT_X2");
        }
    }
